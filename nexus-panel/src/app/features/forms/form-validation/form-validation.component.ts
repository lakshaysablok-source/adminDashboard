      ...
      <div class="grid grid-cols-1 xl:grid-cols-2 gap-4">
        ...
        <div class="card">
          ...
          <form [formGroup]="pwdForm" (ngSubmit)="submitPwd()" class="space-y-3">
            ...
            <!-- Strength indicator — driven by computed signals, no CD lag -->
            @if (pwdValue()) {
              <div class="space-y-2">
...
export default class FormValidationComponent {
  ...
  // Reactive password value — updates only when the field changes, not every CD cycle
  pwdValue = toSignal(this.pwdForm.get('password')!.valueChanges, { initialValue: '' });
  ...
  pwdStrengthColor = computed(() =>
    ['#ef4444', '#f59e0b', '#3b82f6', '#22c55e'][this.pwdStrength() - 1] ?? '#e2e8f0'
  );

  pwdStrengthLabel = computed(() =>
    ['Weak', 'Fair', 'Good', 'Strong'][this.pwdStrength() - 1] ?? ''
  );
